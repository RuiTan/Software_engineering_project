                        ...
                        <div class="column">
                            <div class="ui segments">
                                ...
                                <form class="ui form segment form7">
                                    ...
                                    <div class="ui blue button" onclick="createGroupTask()">创建公共任务</div>
                                    <table class="ui  fluid celled table">
                                        <tbody id="bills">
                                        <tr>
                                            <td class="three wide">任务标题</td>
                                            <td class="eight wide">任务描述</td>
                                            <td class="three wide">创建时间</td>
                                            <td class="two wide">任务状态</td>
                                        </tr>
                                        <c:forEach var="t" items="${groupTask}">
                                            <tr>
                                                <td class="three wide">${t.title}</td>
                                                <td class="eight wide">${t.content}</td>
                                                <td class="three wide">${TimeUtil.stampToDate(String.valueOf(t.createdate))}</td>
                                                <td class="two wide">
                                                    <c:choose>
                                                        <c:when test="${t.enddate <= TimeUtil.currentStamp()}">
                                                            已过期
                                                        </c:when>
                                                        <c:otherwise>
                                                            <c:choose>
                                                                <c:when test="${t.status == \"1\"}">进行中</c:when>
...
<script>
    function createGroupTask() {
        ...
        var time = document.getElementById('time').value + ' 01:01:01';
        var ok = true;
        if (title.length <= 0 ){
            swal({
                title : "Error",
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data == '1'){
                        ...
                        var url = 'window.location.href = \'groupTask\'';
                        setTimeout(url ,2000);
                    }else if (data == '2'){
                        swal({
                            title : "Failed",
